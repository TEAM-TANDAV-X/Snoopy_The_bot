...
async def _(event):
    ...
    animation_chars = [
        "**Heroku Connecting To Latest [Github Build](sameerpanthi/deadly-op-bot)**",
        ...
        "__INFO:tandavbot:Logged in as 557667062__",
...
@bot.on(admin_cmd(pattern=r"ppromote$", outgoing=True))
@bot.on(sudo_cmd(pattern=r"ppromote$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 3
