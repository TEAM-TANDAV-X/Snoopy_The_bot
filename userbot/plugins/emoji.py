...
@bot.on(admin_cmd(pattern=r"sad$"))
@bot.on(sudo_cmd(pattern="sad$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(admin_cmd(pattern=f"2heart$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"2heart$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(sudo_cmd(pattern=f"sex", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 1
...
CmdHelp("animoji").add_command(
  ...
  'cry', None, 'Crying animation🥱'
).add_command(
  'fap', None, 'Cool fapping animation'
).add_command(
  'kiss', None, 'Lets kiss😘'
).add_command(
  'sex', None, 'Wanna do sex wid me?'
).add_command(
